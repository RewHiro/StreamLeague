...
/// エネルギーを集める機能
/// </summary>
public class StrongCollectState : State<StrongAI>
{
    public StrongCollectState(StrongAI strong_ai) :
        base(strong_ai)
    {
    ...
    public override void Enter()
    {
        ...
        if (_targetTransform == null) _stateManager.ChangeState(StrongAI.State.COLLECT);

        _whoHaveEnergy = _targetTransform.GetComponent<WhoHaveEnergy>();

        if ((int)_whoHaveEnergy._whoHave == (int)_type) _stateManager.ChangeState(StrongAI.State.COLLECT);
