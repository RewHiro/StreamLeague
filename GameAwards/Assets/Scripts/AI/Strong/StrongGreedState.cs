...
/// 盗む機能
/// </summary>
public class StrongGreedState : State<StrongAI>
{
    public StrongGreedState(StrongAI strong_ai):
        base(strong_ai)
    {
    ...
    public override void Enter()
    {
        ...
        {
            _stateManager.ChangeState(StrongAI.State.ATTACK);
            return;
        }

        var target = energies.ToArray()[energies.Count() - 1].transform;
        ...
        if (_targetTransform == null) _stateManager.ChangeState(StrongAI.State.ATTACK);

        _whoHaveEnergy = _targetTransform.GetComponent<WhoHaveEnergy>();

        if ((int)_whoHaveEnergy._whoHave == (int)_type) _stateManager.ChangeState(StrongAI.State.ATTACK);
