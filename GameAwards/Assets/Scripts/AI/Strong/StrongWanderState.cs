...
public class StrongWanderState : State<StrongAI>
{

    const float WANDER_TIME = 0.2f;
    const int WANDET_STOP_COUNT = 5;

    public StrongWanderState(StrongAI strong_ai) :
        base(strong_ai)
    {
    ...
    IEnumerator Wander(Vector3 direction)
    {
        ...
        if (_attack.isCanAttack && _attack.isInRange) _stateManager.ChangeState(StrongAI.State.ATTACK);
    }
